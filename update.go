...
import (
	...
	"routre-cli/internal/update"
)

// cmdUpdate implements `routre-cli update`: check for (and optionally apply)
...
func cmdUpdate(args []string, logger *log.Logger) error {
	...
	case 1:
		fmt.Printf("routre-cli %s is newer than %s — nothing to do\n", current, rel.Tag)
		return nil
	case 0:
	...
	if current == "" || current == "dev" {
		fmt.Println("(installing anyway — use routre-cli version afterwards to confirm)")
	}
	...
	fmt.Println("updated to", rel.Tag, "— run 'routre-cli version' to confirm")
	return nil
}
