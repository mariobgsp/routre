...
type Pipeline struct {
	...
	keys       *keystore.Store
	logger     *log.Logger
	// lastPhases holds the per-attempt wall-clock breakdown from the
	// most recent successful eval (stream or non-streaming). Read by
	// the HTTP handler to enrich the request-log entry. Reset at the
	// top of every Stream/Process call so a cache-served request
	// doesn't leak the previous attempt's phases. Mutated only by
	// streamEval/tryEval; read on the same goroutine that called
	// Stream/Process, so no lock needed.
	lastPhases *Phases
}
