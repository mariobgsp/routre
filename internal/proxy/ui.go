...
const uiCSS = `@import url('https://fonts.googleapis.com/css2?family=Press+Start+2P&family=VT323&display=swap');
:root{--paper:#12101f;--paper-3:#0c0a18;--ink:#f0e6d2;--steel:#c9b896;--hairline:#4a3f5c;--accent:#ca8a04;--torch:#ff7a3d;--ok:#7bc96f;--mono:"VT323",ui-monospace,Menlo,Consolas,monospace;--sans:"VT323",ui-monospace,monospace;--px:"Press Start 2P",monospace}
...
// uiJS holds the dashboard client script (saveCfg + setKey), kept separate
// from uiHTML so markup stays readable. No template actions inside.
const uiJS = `async function saveCfg(){
  const el=document.getElementById('cfg'), msg=document.getElementById('cfgMsg');
  msg.textContent=' saving…'; msg.style.color='var(--steel)';
  let body;
  try{ body=JSON.parse(el.value); }catch(e){ msg.textContent=' invalid JSON: '+e.message; msg.style.color='var(--accent)'; return; }
  const r=await fetch('/ui/api/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
  const j=await r.json().catch(()=>({error:'bad response'}));
  if(r.ok){ msg.textContent=' saved ✓'; msg.style.color='#4ade80'; }else{ msg.textContent=' '+ (j.error||r.statusText); msg.style.color='var(--accent)'; }
}
async function setKey(){
  const k=document.getElementById('envKey').value.trim(), v=document.getElementById('envVal').value;
  const msg=document.getElementById('keyMsg');
  if(!k){ msg.textContent=' enter env var name'; msg.style.color='var(--accent)'; return; }
  msg.textContent=' saving…'; msg.style.color='var(--steel)';
  const r=await fetch('/ui/api/env',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({key:k,value:v})});
  const j=await r.json().catch(()=>({error:'bad response'}));
  if(r.ok){ msg.textContent=' saved ✓'; msg.style.color='#4ade80'; document.getElementById('envVal').value=''; }else{ msg.textContent=' '+ (j.error||r.statusText); msg.style.color='var(--accent)'; }
}`

// loopback guard: dashboard + UI APIs accept loopback Host only
// (DNS-rebinding mitigation); all handlers below enforce it.
...
<p class="mut" style="font-size:1rem">Your AI apps talk to <span class="mono">{{.Listen}}</span> (this computer only). Everything you change here saves to <span class="mono">{{.ConfigPath}}</span> and works right away — no restart.</p>

<div class="grid">
<div class="card"><div class="mono mut lbl">Token saver (RTK)</div><div class="stat-v">{{if .RTKEnabled}}ON · {{.RTKLevel}}{{else}}OFF{{end}}</div><div class="mut mono sub">{{.RTKMin}}–{{.RTKMax}} bytes · shrinks repeat tool text</div></div>
<div class="card"><div class="mono mut lbl">Memory (Cache)</div><div class="stat-v">{{if .CacheEnabled}}ON · {{.CacheEntries}} saved answers{{else}}OFF{{end}}</div><div class="mut mono sub">kept {{.CacheTTL}}s · {{.CacheBytes}} bytes</div></div>
<div class="card"><div class="mono mut lbl">Status</div><div class="stat-v">running {{.Uptime}}s</div><div class="mut mono sub">{{.ProviderCount}} providers · {{.TierCount}} groups</div></div>
</div>

<h2>Providers — who answers when</h2>
<p class="mut" style="font-size:1rem">Top group is tried first. If it is busy, routre automatically tries the next. Green ● means the key is saved.</p>
...
<h2>Step 2 — paste your API key</h2>
<div class="card"><div class="kv"><div><label>Key name (e.g. OPENCODE_GO_API_KEY)</label><input id="envKey" placeholder="OPENCODE_GO_API_KEY"></div><div><label>Key value</label><input id="envVal" type="password" placeholder="sk-..."></div></div>
...
<p class="mut mono" style="font-size:.85rem;margin-top:8px">Saved safely to <span class="mono">{{.EnvPath}}</span> (only you can read it). Works immediately.</p></div>

<h2>Step 3 — settings (JSON, optional)</h2>
<p class="mut" style="font-size:1rem">Most people never need this. If you edit, routre checks it first — mistakes are rejected and nothing breaks.</p>
...
<li>Point any AI app at <span class="mono">http://{{.Listen}}</span> — it just works.</li>
<li>Green dot above means ready. Missing? Paste the key in Step 2.</li>
<li>This page costs nothing when idle — see <a href="https://github.com/mariobgsp/routre#benchmarks">Benchmarks</a>.</li>
</ul>
</main>
<script>
` + uiJS + `
...
// template data: uiData feeds the uiHTML template only (no CSS/JS vars).
type uiData struct {
	Listen        string
	ConfigPath    string
	EnvPath       string
...
// handlers: loopback-guarded UI endpoints (page + JSON APIs).
